...
const DEFAULT_TREND_CONFIG: TrendReportConfig = {
  dimensions: [
    ...
    },
    {
      ...
      subItems: [
        { name: 'TrendIndicators', weight: 0.18, dataSource: 'Binance/OKX OHLCV', dataDescription: 'MA交叉(EMA7/25, MA50/200)、MACD、ADX趋势强度', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com', enabled: true },
        { name: 'Momentum', weight: 0.15, dataSource: 'Binance/OKX OHLCV', dataDescription: 'RSI(14/28)、StochRSI、CCI、Williams %R', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com', enabled: true },
        { name: 'Volatility', weight: 0.12, dataSource: 'Binance/OKX OHLCV', dataDescription: '布林带(BB)、ATR(14)、历史波动率(HV)', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com', enabled: true },
        { name: 'VolumeAnalysis', weight: 0.13, dataSource: 'Binance/OKX OHLCV', dataDescription: 'OBV、VWAP、成交量分布、CMF资金流', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com', enabled: true },
        { name: 'SupportResistance', weight: 0.12, dataSource: 'Binance/OKX OHLCV', dataDescription: '斐波那契回撤、枢轴点、关键支撑阻力位', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com', enabled: true },
        { name: 'CandlestickPatterns', weight: 0.10, dataSource: 'Binance/OKX OHLCV', dataDescription: 'K线形态识别(吞没、十字星、锤头等)', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com', enabled: true },
        { name: 'MarketMicrostructure', weight: 0.10, dataSource: 'Binance/OKX OrderBook + Coinglass', dataDescription: '订单簿深度、买卖价差、爆仓数据', apiType: 'REST/WebSocket', apiEndpoint: 'api.binance.com;api.okx.com;api.coinglass.com', enabled: true },
        { name: 'ExecutionLayer', weight: 0.10, dataSource: '内部监控 + Exchange Ping', dataDescription: '滑点(bps)、网络延迟(ms)、API成功率、流动性深度', apiType: 'REST/WebSocket', apiEndpoint: '内部监控系统', enabled: true },
      ],
      coinSpecificItems: {
        BTC: [
          { name: 'HalvingCycle', weight: 0.20, dataSource: 'Blockchain.com / Blockchair', dataDescription: '减半周期阶段、距下次减半天数、历史规律', apiType: 'REST API', apiEndpoint: 'api.blockchain.info;api.blockchair.com', enabled: true },
          { name: 'HashRate', weight: 0.18, dataSource: 'Glassnode / CoinMetrics', dataDescription: '全网算力趋势、挖矿难度调整幅度', apiType: 'REST API', apiEndpoint: 'api.glassnode.com;api.coinmetrics.io', enabled: true },
          { name: 'MinerFlow', weight: 0.15, dataSource: 'Glassnode / CryptoQuant', dataDescription: '矿工收入、矿工流出量、矿工储备', apiType: 'REST API', apiEndpoint: 'api.glassnode.com;api.cryptoquant.com', enabled: true },
          { name: 'MVRV', weight: 0.18, dataSource: 'Glassnode / CoinMetrics', dataDescription: '市场价值/实现价值比率、MVRV Z-Score', apiType: 'REST API', apiEndpoint: 'api.glassnode.com;api.coinmetrics.io', enabled: true },
          { name: 'NUPL', weight: 0.14, dataSource: 'Glassnode / LookIntoBitcoin', dataDescription: '净未实现盈亏、市场情绪阶段', apiType: 'REST API', apiEndpoint: 'api.glassnode.com', enabled: true },
          { name: 'StockToFlow', weight: 0.10, dataSource: 'LookIntoBitcoin / 自建计算', dataDescription: 'S2F模型价格偏差、存量产量比', apiType: 'REST API', apiEndpoint: '自建计算 + api.glassnode.com', enabled: true },
          { name: 'LightningNetwork', weight: 0.05, dataSource: 'mempool.space / 1ML', dataDescription: '闪电网络容量、通道数、节点数增长', apiType: 'REST API', apiEndpoint: 'mempool.space/api;api.1ml.com', enabled: true },
        ],
        ETH: [
          { name: 'GasCongestion', weight: 0.15, dataSource: 'Etherscan / Alchemy', dataDescription: 'Gas价格(Gwei)、待处理交易数、网络拥堵度', apiType: 'REST API', apiEndpoint: 'api.etherscan.io;eth-mainnet.g.alchemy.com', enabled: true },
          { name: 'BurnRate', weight: 0.20, dataSource: 'Etherscan / ultrasound.money', dataDescription: 'EIP-1559每日销毁量、净发行量、通缩/通胀状态', apiType: 'REST API', apiEndpoint: 'api.etherscan.io;ultrasound.money/api', enabled: true },
          { name: 'Staking', weight: 0.20, dataSource: 'Beaconcha.in / Rated.network', dataDescription: '质押总量、质押率、验证者数量、质押APY', apiType: 'REST API', apiEndpoint: 'beaconcha.in/api;api.rated.network', enabled: true },
          { name: 'DeFiTVL', weight: 0.18, dataSource: 'DefiLlama', dataDescription: 'DeFi总锁仓量、Top协议TVL变化', apiType: 'REST API', apiEndpoint: 'api.llama.fi', enabled: true },
          { name: 'L2Activity', weight: 0.15, dataSource: 'L2Beat / DefiLlama', dataDescription: 'L2 Rollup TVL、交易量(Arbitrum/Optimism/Base等)', apiType: 'REST API', apiEndpoint: 'l2beat.com/api;api.llama.fi', enabled: true },
          { name: 'ETH_BTC_Ratio', weight: 0.12, dataSource: 'Binance / CoinGecko', dataDescription: 'ETH/BTC汇率趋势、相对强弱', apiType: 'REST API', apiEndpoint: 'api.binance.com;api.coingecko.com', enabled: true },
        ],
      },
    },
  ],
